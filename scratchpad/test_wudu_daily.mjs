...
// —— 入宗介绍: 无条件稳定触发 ——
t('入宗介绍: 首轮稳定触发', render(base(), '今天天气不错'), ['五毒教·入宗介绍'], ['偷嘴的圣女']);
t('入宗介绍: 已过不复燃', render(base(已入教(), { 地点: { 具体地点: '五毒教·神木殿' } }), '今天天气不错', 0.9).trim(), [], ['<event>']);
// —— 总开关 ——
t('总开关: 非五毒身份→全静默', render(base({}, { 身份: ['散修'] }), '蓝幽月 百草瘴').trim(), [], ['<event>']);
t('总开关: 进行中大事件→静默', render(base({ 标题: '梦茧' }), '蓝幽月 百草瘴').trim(), [], ['<event>']);
// —— 偷糖记(炼气) ——
t('偷糖记: 炼气+关键词触发', render(base(已入教(), {}), '{{user}}路过百草瘴看见蓝幽月'), ['偷嘴的圣女', '偷糖记'], []);
t('偷糖记: 筑基不触发(境界越限)', render(base(已入教(), { 修炼进度: { 境界: '筑基初期' } }), '百草瘴 蓝幽月').trim(), [], ['偷嘴的圣女']);
t('偷糖记: 已做过不复燃', render(base({ 已完成事件: ['五毒教·入宗介绍', '偷糖记'] }), '百草瘴 蓝幽月', 0).trim(), [], ['偷嘴的圣女']);
// —— 试毒场(夜+灵堂) ——
t('试毒场: 夜晚+灵堂触发', render(base({ 已完成事件: ['五毒教·入宗介绍', '偷糖记'] }, { 时间: { 时辰: '子时' } }), '{{user}}深夜路过灵堂'), ['独自试毒'], []);
t('试毒场: 白天灵堂不触发', render(base({ 已完成事件: ['五毒教·入宗介绍', '偷糖记'] }), '{{user}}路过灵堂').trim(), [], ['独自试毒']);
// —— 过去的影子(蓝幽月+修炼) ——
t('过去的影子: 蓝幽月旁修炼触发', render(base({ 已完成事件: ['五毒教·入宗介绍', '偷糖记'] }), '{{user}}在蓝幽月身边打坐修炼'), ['入梦', '过去的影子'], []);
// —— 毒膳堂 ——
t('毒膳堂: 膳堂关键词触发', render(base({ 已完成事件: ['五毒教·入宗介绍', '偷糖记'] }), '{{user}}第一次去膳堂用饭'), ['五毒膳堂'], []);
// —— 蟾佩刮金 ——
t('蟾佩刮金: 账房关键词触发', render(base({ 已完成事件: ['五毒教·入宗介绍', '偷糖记'] }), '{{user}}进了账房看账目'), ['账房', '詹金蟾'], []);
// —— 五部演武·预告(筑基+已完成梦茧) ——
t('演武预告: 筑基+已通梦茧+修炼触发', render(base({ 已完成事件: ['五毒教·入宗介绍', '梦茧'] }, { 修炼进度: { 境界: '筑基后期' } }), '{{user}}修炼出关'), ['五部演武', '五部演武·预告'], []);
t('演武预告: 未完成梦茧不触发', render(base(已入教(), { 修炼进度: { 境界: '筑基后期' } }), '{{user}}修炼出关', 0).trim(), [], ['五部演武']);
// —— 断供(金丹) ——
t('断供: 金丹+宗内触发', render(base(已入教(), { 修炼进度: { 境界: '金丹中期' } }), '{{user}}正在神木殿宗内'), ['断供', '千形宗'], []);
t('断供: 筑基不触发(境界不足)', render(base(已入教(), { 修炼进度: { 境界: '筑基后期' } }), '神木殿 宗内', 0).trim(), [], ['急报']);
