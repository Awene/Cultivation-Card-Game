...
async function readUpload(context: AppContext): Promise<{ bytes: Uint8Array; description: string }> {
...
export async function listPublicWorldbooks(context: AppContext): Promise<Response> {
  ...
  ).bind(...bindings).all<WorldbookPackRow>();
  return context.json({
...
export async function getPublicWorldbook(context: AppContext): Promise<Response> {
  ...
  ).bind(context.req.param('packId')!).first<WorldbookPackRow>();
  if (!row) return context.json({ error: '世界书包不存在或已下架' }, 404);
  return context.json({ pack: publicWorldbook(row, context.env.PUBLIC_BASE_URL) });
}
...
export async function listOwnWorldbooks(context: AppContext): Promise<Response> {
  const rows = await context.env.DB.prepare(
    "SELECT * FROM worldbook_packs WHERE owner_id = ? AND status != 'removed' ORDER BY updated_at DESC",
  ).bind(context.get('user').id).all<WorldbookPackRow>();
  return context.json({ items: rows.results.map(row => publicWorldbook(row, context.env.PUBLIC_BASE_URL)) });
}
...
export async function createWorldbookPack(context: AppContext): Promise<Response> {
  ...
  const usage = await context.env.DB.prepare(
    "SELECT COUNT(*) AS count FROM worldbook_packs WHERE owner_id = ? AND status != 'removed'",
  ...
  try {
    await context.env.DB.prepare(
      ...
      id, user.id, metadata.name, upload.description, metadata.category, metadata.dlcKey,
      JSON.stringify(metadata.relations), metadata.entryCount, objectKey, upload.bytes.byteLength, digest, now, now,
    ).run();
  } catch (error) {
    await context.env.IMAGES.delete(objectKey);
    throw error;
  }
  await writeAudit(context.env, user.id, 'worldbook.create', 'worldbook', id, { dlcKey: metadata.dlcKey });
...
export async function replaceWorldbookContent(context: AppContext): Promise<Response> {
  ...
  try {
    await context.env.DB.prepare(
      ...
      metadata.name, upload.description, metadata.category, metadata.dlcKey,
      JSON.stringify(metadata.relations), metadata.entryCount, objectKey, upload.bytes.byteLength, digest, now, row.id,
    ).run();
  } catch (error) {
...
export async function removeWorldbookPack(context: AppContext): Promise<Response> {
  const row = await ownWorldbook(context, context.req.param('packId')!);
  await context.env.DB.prepare(
    "UPDATE worldbook_packs SET status = 'removed', version = version + 1, updated_at = ? WHERE id = ?",
  ).bind(nowSeconds(), row.id).run();
...
export async function serveWorldbookContent(context: AppContext): Promise<Response> {
  const row = await context.env.DB.prepare(
    "SELECT * FROM worldbook_packs WHERE id = ? AND status = 'published'",
  ).bind(context.req.param('packId')!).first<WorldbookPackRow>();
  if (!row) return context.json({ error: '世界书包不存在或已下架' }, 404);
...
async function worldbookObjectResponse(context: AppContext, row: WorldbookPackRow, publicCache: boolean): Promise<Response> {
  const object = await context.env.IMAGES.get(row.object_key, { onlyIf: context.req.raw.headers });
