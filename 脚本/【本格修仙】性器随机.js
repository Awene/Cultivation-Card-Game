...
(function () {
  ...
  // 是否有女性角色尚未生成性器
  function needsFill(sd) {
    if (!sd || typeof sd !== "object") return false;
    if (isFemale(sd.体质) && _.isEmpty(sd.性器)) return true;
    const rel = sd.关系列表 || {};
    return Object.keys(rel).some((n) => {
      const npc = rel[n];
      return npc && npc.类型 === "人物" && npc.在场 && isFemale(npc.体质) && _.isEmpty(npc.性器);
    });
  }

  async function fillGenitals() {
    let vars;
    try { vars = getVariables(OPT); } catch (e) { return; }
    if (!needsFill(_.get(vars, "stat_data"))) return;
    try {
      await updateVariablesWith((v) => {
        const sd = _.get(v, "stat_data");
        if (!sd || typeof sd !== "object") return v;
        // 主角
        ...
        // 在场 NPC
        const rel = sd.关系列表 || {};
        for (const n of Object.keys(rel)) {
          const npc = rel[n];
          if (npc && npc.类型 === "人物" && npc.在场 && isFemale(npc.体质) && _.isEmpty(npc.性器)) {
      ...
      console.error("[性器随机] 写入失败:", e);
    }
  }

  // 触发:角色首次出场(新回复后 NPC 就位)/ 切换聊天时补齐。
  // 延时 400ms 让 MVU 先完成本轮变量更新,再补性器。
  let _t = null;
  function schedule() {
    if (_t) clearTimeout(_t);
    _t = setTimeout(fillGenitals, 400);
  }
  eventOn(tavern_events.MESSAGE_RECEIVED, schedule);
  eventOn(tavern_events.CHAT_CHANGED, schedule);
  // 脚本加载即尝试一次(补齐当前楼层已在场的女性)
  schedule();
})();
