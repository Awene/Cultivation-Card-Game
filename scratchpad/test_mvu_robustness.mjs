...
assert.equal(commands.length, 3, "未知顶级字段与不存在的删除应被忽略");
assert.equal(commands[0].args[0], "物品");
assert.equal(JSON.parse(commands[0].args[1]), "白玉壶");
assert.equal(commands[1].args[0], "物品");
assert.equal(JSON.parse(commands[1].args[1]), "丹药.A/B", "JSON Pointer 特殊字符应保持在同一个动态 key 中");
assert.equal(commands[2].args[0], '物品["丹药.A/B"].数量', "特殊 key 的后续字段路径不得被二次拆分");
