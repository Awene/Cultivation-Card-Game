...
const ensureSummaryHintStyle = () => {
  ...
  style.textContent = `
    .sa-summary-hint {
      ...
      box-shadow: 0 10px 28px rgba(89, 65, 33, 0.25);
      pointer-events: none;
      text-align: center;
    ...
    .sa-summary-hint.sa-summary-hint-success {
      border-color: rgba(91, 138, 114, 0.7);
      background: rgba(233, 242, 233, 0.97);
    }
    .sa-summary-hint.sa-summary-hint-error {
      border-color: rgba(177, 58, 58, 0.7);
      background: rgba(246, 227, 227, 0.97);
    }
  `;
...
.sa-overlay, .sa-panel, .sa-panel * {
  /* ===== 米黄纸主题（对齐 MVU 状态栏配色）===== */
  --sa-bg:          #efe6cd;   /* 深纸/最底 */
  --sa-bg2:         #f7f1e3;   /* 主面板底 (paper) */
  --sa-bg3:         #faf6ee;   /* 内层浅面 (paper-soft) */
  --sa-surface:     rgba(201,169,110,0.12);  /* 暖金淡层卡面 */
  --sa-surface2:    rgba(201,169,110,0.20);
  --sa-border:      rgba(201,169,110,0.45);  /* 金线 */
  --sa-border2:     rgba(42,37,32,0.12);
  --sa-text:        #2a2520;   /* 墨 */
  --sa-text2:       #5a5249;
  --sa-text3:       #8b8275;
  --sa-accent:      #a07f48;   /* 金(深) 主强调 */
  --sa-accent2:     #c9a96e;   /* 金 */
  --sa-accent-glow: rgba(201,169,110,0.30);
  --sa-teal:        #5b8a72;   /* 玉 成功/正向 */
  --sa-red:         #b13a3a;   /* 朱砂 危险/删除 */
  --sa-red-dim:     rgba(177,58,58,0.12);
  --sa-radius:      12px;
  --sa-radius-sm:   8px;
...
.sa-overlay {
  ...
  background: rgba(42,37,32,0.45); z-index: 10000;
...
.sa-panel {
  ...
  box-shadow: 0 24px 64px rgba(0,0,0,0.7), 0 0 80px rgba(201,169,110,0.08) inset;
  overflow: hidden;
...
.sa-footer {
  ...
  background: rgba(42,37,32,0.06); flex-shrink: 0;
}
...
.sa-input, .sa-select {
  ...
  border: 1px solid rgba(60,50,38,0.12) !important;
...
.sa-select option { background: #f7f1e3 !important; color: #2a2520 !important; }
...
.sa-btn:hover { background: rgba(60,50,38,0.11); border-color: rgba(60,50,38,0.2); color: var(--sa-text); }
.sa-btn:active { transform: scale(0.97); }
.sa-btn-group { display: flex; gap: 8px; flex-wrap: wrap; }

.sa-btn-primary { background: linear-gradient(135deg, var(--sa-accent), #c9a96e); color: #2a2520; border-color: transparent; box-shadow: 0 2px 12px rgba(201,169,110,0.35); }
.sa-btn-primary:hover { background: linear-gradient(135deg, var(--sa-accent2), #d8bd82); box-shadow: 0 4px 18px rgba(201,169,110,0.5); }
.sa-btn-danger { background: var(--sa-red-dim); border-color: rgba(177,58,58,0.3); color: var(--sa-red); }
.sa-btn-danger:hover { background: rgba(177,58,58,0.25); border-color: rgba(177,58,58,0.5); color: #b13a3a; }
.sa-btn-success { background: rgba(91,138,114,0.12); border-color: rgba(91,138,114,0.3); color: #3d6b54; }
.sa-btn-success:hover { background: rgba(91,138,114,0.22); border-color: rgba(91,138,114,0.5); color: #3d6b54; }
.sa-btn-warn { background: rgba(201,169,110,0.12); border-color: rgba(201,169,110,0.3); color: #a07f48; }
.sa-btn-warn:hover { background: rgba(201,169,110,0.22); border-color: rgba(201,169,110,0.5); color: #8a6a2e; }
...
.sa-no-trans-input { all: unset; width: 110px; height: 24px; padding: 2px 6px; margin-left: 4px; font-size: 12px; border: 1px solid rgba(60,50,38,0.2); border-radius: 4px; background: rgba(60,50,38,0.06); color: var(--sa-text); box-sizing: border-box; vertical-align: middle; }
.sa-no-trans-input:focus { border-color: var(--sa-accent); outline: none; }

input[type="checkbox"], input[type="radio"] {
  all: unset; width: 16px; height: 16px; border: 2px solid rgba(60,50,38,0.2);
...
.sa-progress-bar { width: 100%; height: 5px; background: rgba(60,50,38,0.07); border-radius: 3px; overflow: hidden; margin-top: 4px; }
...
.sa-entry-item:hover { background: rgba(201,169,110,0.07); }
.sa-entry-item.sa-entry-selectable { cursor: pointer; }
.sa-entry-item.sa-entry-selectable:hover { background: rgba(201,169,110,0.12); }
...
.sa-mega-entry-item { background: rgba(91,138,114,0.05); border-left: 3px solid var(--sa-teal); }
.sa-mega-entry-item:hover { background: rgba(91,138,114,0.10); }
...
.sa-entry-badge-mega { background: rgba(91,138,114,0.15); color: #3d6b54; border: 1px solid rgba(91,138,114,0.3); }
.sa-entry-badge-disabled { background: rgba(60,50,38,0.06); color: var(--sa-text3); border: 1px solid rgba(60,50,38,0.1); }

.sa-selection-controls { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; border-bottom: 1px solid var(--sa-border2); background: rgba(201,169,110,0.04); }
...
const showSettingsPopup = errorCatched(async () => {
  ...
  const fitOverlay = () => {
    ...
    overlay.style.cssText = `
      ...
      background: rgba(42,37,32,0.45) !important; backdrop-filter: blur(8px) !important; -webkit-backdrop-filter: blur(8px) !important;
...
// 注册脚本按钮（visible:false —— 不再于底部脚本按钮栏显示「总结设置」，改由状态栏 MVU 面板里的按钮触发）
replaceScriptButtons([{ name: CONFIG.MAIN_BUTTON_NAME, visible: false }]);

// 点击按钮打开设置面板（保留：底部按钮若被重新启用仍可用）
eventOn(getButtonEvent(CONFIG.MAIN_BUTTON_NAME), showSettingsPopup);

// 跨上下文触发口：状态栏(消息 iframe)里的按钮 eventEmit('本格修仙:打开总结') 即可打开总结面板
eventOn('本格修仙:打开总结', showSettingsPopup);
